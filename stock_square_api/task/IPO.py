...
def IPONewsMethod():
    ...
    user_agent = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.58 ' \
                 'Safari/537.36 '
    config = Config()
    ...
    for i in links:
        ...
        image = i.find("img").attrs['data'].strip()
        source = "Moneycontrol"
        ...
        data = (title, desc, new_time, urlLink, image, source, read_time)
        lst.append(data)

    url = 'https://www.moneycontrol.com/news/business/ipo/page-2'
    page = requests.get(url)
    soup = BeautifulSoup(page.text, "html.parser")
    links = soup.find('ul', attrs={'id': 'cagetory'}).find_all('li', attrs={'class': 'clearfix'})
    for i in links:
    ...
    url = 'https://www.moneycontrol.com/news/business/ipo/page-3'
    page = requests.get(url)
    soup = BeautifulSoup(page.text, "html.parser")
    links = soup.find('ul', attrs={'id': 'cagetory'}).find_all('li', attrs={'class': 'clearfix'})
    for i in links:
